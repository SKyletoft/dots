;;; custom-monokai.el --- A simple theme based on the Monokai Pro Sublime color schemes
...
(defvar custom-monokai-theme-default-colors
      ...
      (let* ((key       (car  spec))
        ...
        (cond
         ((and (memq key '(:box :underline)) (listp value))
          (push (custom-monokai-theme-transform-spec value colors) output))
         (color
          (push color output))
         (t
          (push value output)))
...
(defun custom-monokai-theme-transform-face (spec colors)
  "Transform a face `SPEC' into an Emacs theme face definition using `COLORS'."
  (let* ((face             (car spec))
         (definition       (cdr spec)))

    (list face `((t ,(custom-monokai-theme-transform-spec definition colors))))))

(defun custom-monokai-theme-set-faces (theme-name colors faces)
  ...
  (apply 'custom-theme-set-faces theme-name
         (mapcar #'(lambda (face)
                     (custom-monokai-theme-transform-face face colors))
                 faces)))

(defun custom-monokai-theme-define (theme-name theme-colors)
  "Define the faces for a custom-monokai colorscheme given a `THEME-NAME' and a plist of `THEME-COLORS'."
  (custom-monokai-theme-set-faces
   theme-name
   theme-colors
   '(
;;; Built-in
...
;;;; basic colors
     ...
     (default                                      :foreground al-fg :background al-bg)

     (fringe                                       :background al-bg)
     ...
     (minibuffer-prompt                            :foreground al-fg)
     (region                                       :background bg+2)
     (shadow                                       :foreground fg-2)
     (secondary-selection                          :background bg+2)
     (trailing-whitespace                          :foreground al-fg :background red)
...
;;;; font-lock
     ...
     (font-lock-variable-name-face                 :foreground al-fg)
     (font-lock-warning-face                       :foreground orange)

;;;; isearch
     (match                                        :foreground yellow :background al-bg :inverse-video t)

     ;; TODO: Revisit this - doesn't seem to map properly onto tmThemes
     (isearch                                      :foreground al-bg :background yellow :weight bold)
...
;;;; term
     (term-color-black                             :foreground black       :background al-black-l)
     (term-color-blue                              :foreground al-blue-l   :background al-blue)
     (term-color-cyan                              :foreground al-cyan-l   :background al-cyan)
     (term-color-green                             :foreground al-green-l  :background al-green)
     (term-color-magenta                           :foreground al-purple-l :background al-purple)
     (term-color-red                               :foreground al-red-l    :background al-red)
     (term-color-white                             :foreground al-grey     :background white)
     (term-color-yellow                            :foreground al-yellow-l :background al-yellow)
...
;;;; diff-mode
     ...
     (diff-header                                  :background al-bg)
     (diff-file-header                             :background bg+1)
     (diff-hunk-header                             :foreground pink :background al-bg)
...
;;;; ivy
     ...
     (ivy-current-match                            :foreground al-bg :background yellow)
     (ivy-cursor                                   :foreground al-bg :background al-fg)
     ...
     (ivy-minibuffer-match-face-1                  :foreground al-fg :background bg+1)
     (ivy-minibuffer-match-face-2                  :foreground al-fg :background bg+2)
     (ivy-minibuffer-match-face-3                  :foreground al-fg :background purple)
     (ivy-minibuffer-match-face-4                  :foreground al-fg :background pink)
     (ivy-minibuffer-match-highlight               :foreground fg-4 :background bg+1)
     (ivy-modified-buffer                          :foreground al-fg)
     (ivy-modified-outside-buffer                  :foreground al-fg)
     (ivy-org                                      :foreground bg+1 :slant italic)
     (ivy-prompt-match                             :foreground al-bg :background yellow)
...
(custom-monokai-theme-define 'custom-monokai custom-monokai-theme-default-colors)
(provide-theme 'custom-monokai)

(provide 'custom-monokai)

;;; custom-monokai.el ends here
